...
export const applyPromo = createOcAsyncThunk<RequiredDeep<Order>, string>(
  'ocCurrentOrder/addPromotion',
  async (promoCode, _ThunkAPI) => {
    const { ocCurrentOrder } = _ThunkAPI.getState();
    const orderId = ocCurrentOrder.order ? ocCurrentOrder.order.ID : undefined;
    await Orders.AddPromotion('All', orderId, promoCode);
    const updateOrder = await Orders.Get('All', orderId);
    return updateOrder;
  }
);
...
>('ocCurrentOrder/saveShippingAddress', async (request, ThunkAPI) => {
  ...
  await IntegrationEvents.Calculate('All', orderId);

  return IntegrationEvents.GetWorksheet('Outgoing', orderId);
});

export const patchOrder = createOcAsyncThunk<RequiredDeep<Order>, Partial<Order>>(
  'ocCurrentCart/patch',
  async (partialOrder, ThunkAPI) => {
    const { ocCurrentOrder } = ThunkAPI.getState();
    const orderID = ocCurrentOrder.order.ID;
    const patchedOrder = await Orders.Patch('All', orderID, partialOrder);
