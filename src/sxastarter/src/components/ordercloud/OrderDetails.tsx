...
import { Address, LineItem, LineItems, Order, Orders, Shipment } from 'ordercloud-javascript-sdk'
...
export const Default = (props: ComponentProps): JSX.Element => {
  ...
  const canReturn = useMemo(() => {
    return (
      ...
      !!lineItems.filter((li: LineItem) => li.Product.Returnable).length
    )
  }, [order, lineItems])
  ...
  useEffect(() => {
    async function initialize() {
      ...
      const shipments = await Orders.ListShipments('All', orderID)
      setOrder(order)
      setBillingAddress(billingAddress)
      setLineItems(lineItems.Items)
      setShipments(shipments.Items)
      setLoading(false)
    }
    initialize()
      ...
      {loading ? (
        ...
        >
          <Spinner
            ...
            speed=".8s"
          />
          <Text whiteSpace="nowrap">Gathering Order Details...</Text>
        </VStack>
      ) : (
        <>
          <Container
                    ...
                    >
                      ...
                      >
                        {canReturn && (
                          <Button
                            variant="outline"
                            onClick={returnModalDisclosure.onOpen}
                          >
                            Start a Return
                          </Button>
                        )}
                          ...
                          >
                            ...
                            <Tbody>
                              {shipments.map((shipment: Shipment) => (
                                <Tr key={shipment.ID}>
                                  <Td>{shipment.ID}</Td>
                                  <Td>{formatDate(shipment.DateShipped)}</Td>
                                  <Td>{shipment.Shipper}</Td>
                                  <Td>
                                    <Badge>{shipment.TrackingNumber}</Badge>
                                  </Td>
                                </Tr>
                              ))}
                            </Tbody>
